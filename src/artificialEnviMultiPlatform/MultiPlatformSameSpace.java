...
public class MultiPlatformSameSpace {
	...
	public static void main(String[] args) throws IOException {
		...
		String mainDir = outputDir + "output\\multiPlatform\\SameSpace\\" + area + "sqkm\\" ;
		String probabilityFilePath = mainDir + "multi-same-" + area + "sqkm_probabilities.csv";
...
//						passengers = Methods.defaultOpDinfer(passengers, vehicles, operators, reachMeasure);
						...
						for (int i = 0 ; i < passengers.size() ; i++ )
						{
							...
							int multiHmngPenaltySwch = 0;
							int [] neighbours = new int [operators.size()];
							...
							for (int j = 0 ; j < vehicles.size() ; j++ )
							{
								...
								if (Math.abs(passengerCoord.getX() - vehicleCoord.getX()) <= reachMeasure && Math.abs(passengerCoord.getY() - vehicleCoord.getY()) <= reachMeasure )
								{
									...
									{
										neighbours[vehicles.get(j).getOperator() - 1]++;
										passengers.get(i).neighbour++;
										 
										if (distance < finalDist && vehicles.get(j).capacity > 0)
										{
											if (vehicles.get(j).getOperator() == passengers.get(i).defaultOperator)
											{
												finalDist = distance;
												passengers.get(i).setMtcheVehID(j);
												multiHmngPenaltySwch = 0;
											}
											else
											{
												finalDist = distance;
												passengers.get(i).setMtcheVehID(j);
												multiHmngPenaltySwch = 1;
											}
							...
							if (neighbours. > passInterestThres || passengers.get(i).utility > passUtilThres )//passengers.get(i).neighbour
							{
							...
							if (passengers.get(i).getInterest() == 1 && passengers.get(i).mtcheVehID > -1 )
							{
								...
								double util = (reachMeasure - finalDist) / reachMeasure * potentialUtil - multiHmngPenaltySwch * multiHmngPenalty;
