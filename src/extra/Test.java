...
public class Test {
	...
	public static void main(String[] args) throws IOException {
		...
		String probWritingFile = mainDir + area + "sqkm_probabilities.csv" ;
		...
		BufferedWriter probabilityWriter =  new BufferedWriter(new FileWriter(new File(probWritingFile), false));
		probabilityWriter.write("Area_sqKm,Population,Population_distribution,Vehicles_added_each_iteration,Vehicles_distribution");
		for (int o = 0 ; o < operators.size() ; o++)
		probabilityWriter.write(",SP_" + operators.get(o).getMarketShare() + "_" + operators.get(o).getName());
		...
		{
			for (int v = 10; v < 41 ; v+=100)
			{
				passengers.clear();
				vehicles.clear();
				passengerNumber = p * area;
				allIterations = 0; // counting all instances
...
//				int prob20Per = 0;
				...
				for (int probItcounter = 0 ; probItcounter < probIteration ; probItcounter++)
				{
					...
					String aggregatedResultFile  = dir + "P" + passengerNumber + passDist + "-V" + v + vehDist + "-" + vehicleCapacity + "C" + ".csv";
					String matchedVehicleFile = dir + "matchedVehicles-P" + passengerNumber + passDist + "-V" + v + vehDist + "-" + vehicleCapacity + "C" + ".csv";
		
					BufferedWriter aggregationWriter = new BufferedWriter(new FileWriter(new File(aggregatedResultFile), false));
					aggregationWriter.write("Iteration,Passengers' mean utility,Vehicles' mean utility,"
					...
					BufferedWriter matchedVehicleWriter = new BufferedWriter(new FileWriter(new File(matchedVehicleFile), false));
					matchedVehicleWriter.write("Iteration");
					for (int o = 0 ; o < operators.size() ; o++)
					{
						matchedVehicleWriter.write("," + operators.get(o).getName());
					}
					//========================================================================================
...
//						}
					    ...
					    for (int i = 0 ; i < vehicles.size(); i ++)
						{
					    	int operatorsIndex = vehicles.get(i).operator - 1 ;
					    	vehicleUtilSum[operatorsIndex] += vehicles.get(i).utility;
...
//						double matchedVehPercent = (double) (vehicles.size() - unmatchedVehicles)/vehicles.size() * 100;
						...
						aggregationWriter.write(k + "," + meanPassengersUtil + "," + meanVehUtil + "," 
								+ (double)interestedPassengers/passengers.size() * 100 + "%," 
								+  matchedPassPercent + "%," + matchedVehicles[0] / 1 + "%," + interestedPassengers //CORRECT
								+ "," + matchedPassengers + "," + matchedVehicles + "," 
								+ vehicles.size() + "\n");
						 
					    
					    
					    matchedVehicleWriter.write("\n" + k);
						for (int o = 0 ; o < operators.size() ; o++)
						{
							matchedVehicleWriter.write("," + matchedVehicles[o]);
						}
...
//				System.out.println(p + " passengers and " + v + " vehicles: " + successProb + "%");
				probabilityWriter.write("\n" + area + "," + passengerNumber + "," + passDist + "," + v + "," + vehDist);
				for (int o = 0 ; o < operators.size() ; o++)
					probabilityWriter.write( "," + probability[o]/allIterations * 100);
				probabilityWriter.write("\n");
